...
# root_dir = '../../../data2/'
root_dir = '/media/store/krs/build/athena/test/test_spherical_slice'
...
x2min = 1.5533430342749532
x2max = 1.5882496193148399
x2dims = 3
...
x3a = []
for i in range(0, x3dims):
       x3a.append(float(i)*x3max/float(x3dims-1))
# print(len(x3a), x3a)
# quit()
azimuths = np.radians(x3a)

r, phi = np.meshgrid(x1a, azimuths)

# for field lines:
num = 8  # number of field lines
phi_space = float(x3max)/num
dx1a = x1a[2] - x1a[1]
dx3a = x3a[2] - x3a[1]
dt = 0.0002
maxt = 20000
...
ngh = 0  # number of ghost cells. default is 2
...
for ii in range(n_end+1):
       ...
       data = athena_read.athdf(file_no, quantities=['vel1', 'Bcc1', 'Bcc3', 'rho'], 
              level=level, num_ghost=ngh)

       # print(data)
       v1 = data['vel1'][:,x2_slice,:]
       b1 = data['Bcc1'][:,x2_slice,:]
       b3 = data['Bcc3'][:,x2_slice,:]
       dens = data['rho'][:,x2_slice,:]
       # print(v1.shape, b1.shape, b3.shape, dens.shape)
...
#       print(data)
       ...
       dens_norm = np.ndarray((x3dims, x1dims))

       for i in range(0, x3dims):
              for j in range(0,x1dims):
                     dens_norm[i,j] = dens[i+ngh,j+ngh]*x1a[j]**2.
       # plot field lines
       ...
       for i in range(0,num):
              ...
              while temp_r <= x1max:
                     ...
                     # pause
                     dr  = b1[int(round(th_index))-1+ngh, int(r_index)+ngh] * dt
                     dth = b3[int(round(th_index))-1+ngh, int(r_index)+ngh] * dt / temp_r *180./pi
       ...
       pcm = ax.contourf(phi, r, dens_norm,  extend='max', fontsize=25,\
              levels = np.linspace(0,max_n,150), cmap ='coolwarm', vmin=0.0, vmax=max_n, yunits ='AU')


       # 1AU circle
       ax.plot(np.linspace(0,2*pi,x3dims), [1.0]*x3dims, 'k', linewidth=2.5)
